...
// ─── KPI CARD (MATCHING SCREENSHOT 2026-06-01 113054.PNG EXACTLY) ───────────────────
function KPICard({
  ...
  decorationSvg?: React.ReactNode; // Dedicated bottom-right sketchy decorative asset
  bg: string;
}) {
  return (
    <div style={{
      ...
      {/* 1. LEFT SIDE: Main Interactive Core Icon */}
      <div style={{ 
        display: "flex", 
        alignItems: "center", 
      ...
      {/* 2. CENTER CONTENT: Vertically stacked labels and values */}
      <div style={{ 
        display: "flex", 
        ...
        {/* Mid: Large Styled Numeric Display Value */}
        <div style={{ 
          fontSize: "28px", 
          fontWeight: "600", 
          color: "#1a150e", 
        ...
        {/* Bottom: Micro Context Status Caption Label */}
        <div style={{ 
          fontSize: "11px", 
      ...
      {/* 3. RIGHT SIDE: Absolute Corner Sketch Decoration Icon Doodles */}
      {decorationSvg && (
        <div style={{ 
          position: "absolute", 
          bottom: "8px", 
...
// ─── RESPONSE GROWTH LINE CHART (EXPANDED BLUEPRINT ASPECT RATIO) ───────────────────
function ResponseGrowthChart({ forms }: { forms: TopForm[] }) {
  // Safe processing backup array mapping pipeline
  const data = useMemo(() => {
  ...
  // OPTIMIZED: Aspect ratio expanded to full widescreen view matrix limits
  const W = 750;
  const H = 240;
  
  // Padding boundaries spaced exactly out for the labels to rest cleanly
  const pL = 45; 
  const pR = 25; 
  const pT = 30; 
  const pB = 65; 
  ...
  const maxV = Math.max(...vals, 400); // Fixed standard reference ceiling window max (matches 400 cap in image)
  const minV = 0;

  // Compute exact scaling point matrices tracking coordinate vectors
  const pts = data.map((d, i) => ({
    x: pL + (i / Math.max(data.length - 1, 1)) * cW,
    y: pT + cH - ((d.totalResponses - minV) / (maxV - minV)) * cH,
    label: d.title.replace(/🔐|☀️|🎮|🎌|🚀/g, "").trim(), // Cleans emoji strings safely for display alignment
    val: d.totalResponses,
  }));

  // Build the main spline path alongside a secondary offset path to recreate a true double-ink scribble stroke
  ...
  // Custom baseline rules tracking tick indexes matches image structure (0, 100, 200, 300, 400)
  const ticks = [0, 100, 200, 300, 400];

  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", overflow: "visible" }}>
      <defs>
        {/* Soft backplate color fill wash */}
      ...
      {/* Sketched grid backdrop line dividers loops */}
      {ticks.map((v) => {
        ...
        return (
          <g key={v}>
            <line 
              ...
              x2={W - pR} 
              y2={ty} 
              stroke={v === 0 ? "rgba(45,36,22,0.22)" : "rgba(45,36,22,0.05)"} 
      ...
      {/* Interactive Node Coordinates Rings and Foreign Labels Group Map */}
      {pts.map((p, i) => (
        <g key={i}>
          {/* Floating numeric metric text centered above coordinate node ring */}
          <text 
            x={p.x} 
            y={p.y - 12} 
            textAnchor="middle" 
          ...
          {/* Node intersection concentric bullet tracks */}
          ...
          {/* X-Axis Description Labels Form Title - wrapped beautifully via foreignObject box bounds */}
...
// ─── ALL FORMS COMPACT SCROLLABLE TABLE (FIXED CLAMP & CONTAINER SCROLL) ───
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
        ...
        <div style={{ position: "relative", width: "160px" }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "5px 8px 5px 26px", border: "1px solid rgba(45,36,22,0.15)", borderRadius: "6px", fontSize: "11px", fontFamily: "'Nunito', sans-serif", fontWeight: 600, color: "#2d2416", backgroundColor: "rgba(255,255,255,0.7)", outline: "none", boxSizing: "border-box" }}
          />
        </div>
      </div>

      {/* FIXED: Internal Scrollable Box restricts height but lets you view all data */}
...
// ─── TOP PERFORMING FORMS (INFINITE RANKING SYSTEM — NO FOOTER BUTTON) ───
function TopPerformingForms({ forms }: { forms: TopForm[] }) {
  // Sort responses from highest to lowest across all forms available
  ...
  return (
    <div style={{ backgroundColor: "#FFFDF9", border: "1px solid #e1dbcf", borderRadius: "16px", padding: "16px 20px", fontFamily: "'Nunito', sans-serif", height: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column" }}>
      ...
      {/* Scrollable listing box dynamically scales numerical indices past rank 3 */}
        ...
        {sorted.map((form, i) => (
          <div key={form.id} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ 
              ...
              alignItems: "center", 
              justifyContent: "center", 
              backgroundColor: i < 3 ? `${ringColors[i]}22` : "rgba(45,36,22,0.05)", 
              border: `1.5px solid ${i < 3 ? ringColors[i] : "rgba(45,36,22,0.2)"}`, 
              fontSize: i < 3 ? "14px" : "11px", 
              fontWeight: 800, 
              color: "#2d2416", 
              flexShrink: 0 
            }}>
              {/* FIXED: Drops medals for top 3, then perfectly outputs sequential code index strings */}
              {i < 3 ? medals[i] : i + 1}
            </div>
...
// ─── RECENT ACTIVITY TIMELINE (EXHAUSTIVE REAL-DATA LOG STREAM) ───
function RecentActivity({ forms }: { forms: Form[] }) {
  // Sorts and maps all items present in the backend array dataset directly
  const published = [...forms]
  ...
  return (
    <div style={{ backgroundColor: "#FFFDF9", border: "1px solid #e1dbcf", borderRadius: "16px", padding: "16px 20px", fontFamily: "'Nunito', sans-serif", height: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column" }}>
      ...
      {/* FIXED: Scrolled stream division outputs entire dataset list matching data feed limits */}
...
const stats = useMemo(() => {
  ...
  const convRate =
    summary.totalViews > 0
      ? ((summary.totalResponses / summary.totalViews) * 100).toFixed(1)
      : "0.0";
  ...
  // useFormList is an infinite query — data lives in pages[0].result.data.forms
const forms = useMemo<Form[]>(() => {
  if (!formsData?.pages?.length) return [];

  return formsData.pages.flatMap(
    (page: any) => page.forms ?? []
  );
}, [formsData]);
...
return (
    <div style={{
      ...
      backgroundImage: "url('/dashboard/BG.png')", // Sets your background illustration image as the frame
      backgroundSize: "100% 100%", backgroundPosition: "center", backgroundRepeat: "no-repeat",
      position: "fixed", top: 0, left: 0, overflow: "hidden", boxSizing: "border-box"
    }}>
      {/* Subtle paper texture definitions */}
      <style>{`
      ...
      {/* ── INTERNAL 80% FIXED SKETCH STAGE WRAPPER ── */}
      <div style={{
        position: "absolute", top: 0, left: 0, width: "125vw", height: "125vh",
        display: "flex", transform: "scale(0.8)", transformOrigin: "top left",
...
{/* ── KPI CARDS ROW (CHARACTERS SLEEPING POSITIONING ARRANGEMENT) ── */}
<div 
  style={{ 
    display: "grid", 
    ...
    gap: "28px", 
    marginBottom: "22px",
    position: "relative" /* Provides an overarching reference anchor for absolute coordinate assets */
  }}
>
  ...
  <KPICard
    ...
    decorationSvg={
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#c07a00" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: "rotate(-8deg)" }}>
  ...
  />
    ...
    <KPICard
      ...
      bg="#ede8f9"
      iconSvg={
        <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#7c6fd4" strokeWidth="1.3" strokeLinecap="round">
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
          <polyline points="22,6 12,13 2,6"/>
        </svg>
      }
      decorationSvg={
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="rgba(124, 111, 212)" strokeWidth="1.2">
          <path d="M2 18 Q 8 22, 12 14 T 22 10" strokeDasharray="2,2" />
          <path d="M18 6h5v4h-5z" fill="none" />
        </svg>
      }
    />
  ...
  {/* CARD 2: TOTAL RESPONSES (WITH EXTERNAL OVERLAY POSITIONING FRAME) */}
  <div style={{ position: "relative", display: "flex", flex: 1 }}>
  {/* ── SLEEPING BOY IMAGE POSITIONED EXACTLY ON THE CARD TOP BORDER ── */}
    <div 
      style={{ 
        position: "absolute",
        bottom: "-60px",         /* Negative offset pulls him straight up to sleep on the top border line */
        left: "50%",
        transform: "translateX(-50%)", /* Centers him perfectly relative to Card 2 width margins */
        width: "440px", 
        height: "500px",
        zIndex: 20,           /* Keeps character stacked cleanly on top of card shadows */
        pointerEvents: "none"
      }}
    >
      <Image
        src="/dashboard/boy.png"
        alt="Scribbler character sleeping on top of total responses card widget"
        fill
        priority
  ...
  <KPICard
    ...
    bg="#dff0ff"
    iconSvg={
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#1a73c7" strokeWidth="1.3" strokeLinecap="round">
        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
        <circle cx="12" cy="12" r="3"/>
      </svg>
    }
    decorationSvg={
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(26, 115, 199)" strokeWidth="1.3">
  ...
  <KPICard
    ...
    bg="#e8f5e9"
    iconSvg={
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#2e7d32" strokeWidth="1.3" strokeLinecap="round">
        <circle cx="12" cy="12" r="10"/>
        <path d="M16 8l-8 8"/><path d="M8 8h8v8"/>
      </svg>
    }
    decorationSvg={
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(46, 125, 50)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 19 Q 12 18, 18 6" />
...
<div style={{ display: "grid", gridTemplateColumns: "900px 470px", gap: "38px", marginBottom: "24px", width: "1258px", boxSizing: "border-box" }}>
  ...
  <div 
    style={{ 
      ...
      display: "flex",
      flexDirection: "column",
      height: "300px", /* Equalized height prevents bottom dead spaces */
      boxSizing: "border-box"
    }}
  >
    <div style={{backgroundColor:"#FFFDF9", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        <span style={{ fontSize: "16px" }}>🌿</span>
        <h3 style={{ fontSize: "16px", fontWeight: 800, color: "#1a150e", margin: 0, fontFamily: "'Nunito', sans-serif" }}>
          Response Growth
        </h3>
      </div>
      <div 
  ...
  {/* BLOCK 2: FORMS VOLUME PIE MATRIX */}
  <div 
    style={{ 
      backgroundColor: "#FFFDF9", 
      border: "1.5px solid #2d2416", 
      borderRadius: "16px", 
      padding: "24px", 
      boxShadow: "3px 3px 0px #2d2416",
      display: "flex",
      flexDirection: "column",
      height: "240px", /* Matches parent height bounding perfectly */
      boxSizing: "border-box"
    }}
  >
    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "20px" }}>
      <span style={{ fontSize: "16px" }}>🌿</span>
      <h3 style={{ fontSize: "15px", fontWeight: 800, color: "#1a150e", margin: 0, fontFamily: "'Caveat', cursive" }}>
        Forms Volume Breakdown 🎨
      </h3>
    </div>
...
{/* ── BOTTOM SECTIONS LAYOUT GRID (FIXED VIEWPORT HEIGHT — MATCHES IMAGE PROPORTIONS) ── */}
<div 
  style={{ 
    display: "grid", 
    gridTemplateColumns: "620px 300px 300px", /* Balanced cleanly inside your 125vw viewport wrapper bounds */
    gap: "28px", 
    width: "1248px",
    boxSizing: "border-box",
    marginTop: "4px"
