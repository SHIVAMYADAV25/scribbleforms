...
function LineChart({ data }: { data: { date: string; count: number }[] }) {
  ...
  const yTick = (v: number) => pT + cH - (v / maxV) * cH;
  const ticks = [0, Math.round(maxV / 2), maxV];

  const fmtDate = (d: string) => {
    const [, m, day] = d.split("-");
    const months = ["", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    return `${months[parseInt(m ?? "1")] ?? ""} ${parseInt(day ?? "1")}`;
  };

  const step = Math.max(1, Math.ceil(data.length / 7));
  const xLabelPts = pts.filter((_, i) => i % step === 0 || i === pts.length - 1);

  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", overflow: "visible" }}>
      <defs>
        <linearGradient id="areaGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#634cc9" stopOpacity="0.15" />
...
function CompletionFunnel({ data, dropOffRate }: { data: { stage: string; count: number }[]; dropOffRate: number }) {
  ...
  const conv12 = stages[0] && stages[1] ? Math.round((stages[1].count / stages[0].count) * 100) : null;
  const conv23 = stages[1] && stages[2] ? Math.round((stages[2].count / stages[1].count) * 100) : null;
...
export default function AnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const kpiData = useMemo(() => {
    ...
    const calcPct = (curr: number, p: number) => p === 0 ? 100 : Math.round(((curr - p) / p) * 100);
